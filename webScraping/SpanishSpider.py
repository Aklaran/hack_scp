...
class SpanishSpider(Spider):
    name = 'spanishSpider'
    allowed_domains = ['scp-int.wikidot.com', 'www.wikidot.com', 'scp-es.com'] # don't forget to update this
    start_urls = [ "http://scp-int.wikidot.com/es-hub" ]

    englishDocumentsFolder = "englishFromSpanish/"
    spanishDocumentsFolder = "spanishOriginals/"
    englishBaseUrl = "http://scp-int.wikidot.com"
    spanishBaseUrl = "http://scp-es.com"
    englishToken = '610da9ba2f8fa1ca5ef14c7488cdfc16'
    spanishToken = 'd6be53a2948f8f5eb10d764ecc24c198'
...
#         rows = rows[3:5] # limit rows when testing to save time
    ...
    def parseEnglishAuthorPage(self, response):
        ...
        request = Request(self.spanishBaseUrl + item['href'], callback=self.parseSpanishPage)
        request.meta['data'] = item

        return request

    def parseSpanishPage(self, response):
        item = response.meta['data']

        item['spanishRating'] = response.xpath("//span[@class='rate-points']/span/text()").extract()

        # html
        f = open(self.spanishDocumentsFolder + item["scpId"] + ".html", "w")
        f.write(response.text)
        f.close()

        # txt
        f = open(self.spanishDocumentsFolder + item["scpId"] + ".txt", "w")
        for paragraph in response.xpath("//div[@id='page-content']//text()"):
            f.write(paragraph.extract())
        f.close
        ...
        # open revision history tab and get the initial author + date
        # important that you have wikidot_token7 in both the formdata and the cookie!
        request = FormRequest(self.spanishBaseUrl + '/ajax-module-connector.php',
                                callback=self.parseSpanishAuthorAndDate,
                                method='POST',
                                formdata={'page_id': pageId, 'moduleName': 'history/PageRevisionListModule', 'wikidot_token7': self.spanishToken, 'page': '1', 'perpage': '20'},
                                cookies={"wikidot_token7": self.spanishToken})
        request.meta['data'] = item

        return request

    def parseSpanishAuthorAndDate(self, response):
        item = response.meta['data']
        ...
        item['spanishDate'] = initialRevision.xpath('td[6]/span/text()').extract_first()
        item['spanishAuthor'] = initialRevision.xpath('td[5]/span/a[2]/text()').extract_first()

        # now move to the spanish author page to get their karma
        authorUrl = initialRevision.xpath('td[5]/span/a[2]/@href').extract_first()
        request = Request(authorUrl, callback=self.parseSpanishAuthorPage)
        request.meta['data'] = item

        return request

    def parseSpanishAuthorPage(self, response):
        item = response.meta['data']
        ...
        item['spanishAuthorKarma'] = karmaString
