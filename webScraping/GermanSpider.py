...
class GermanSpider(Spider):
    name = 'germanSpider'
    allowed_domains = ['scp-int.wikidot.com', 'www.wikidot.com', 'scp-wiki-de.wikidot.com'] # don't forget to update this
    start_urls = [ "http://scp-int.wikidot.com/de-hub" ]

    englishDocumentsFolder = "englishFromGerman/"
    germanDocumentsFolder = "germanOriginals/"
    englishBaseUrl = "http://scp-int.wikidot.com"
    germanBaseUrl = "http://scp-wiki-de.wikidot.com"
    englishToken = '610da9ba2f8fa1ca5ef14c7488cdfc16'
    germanToken = 'd6be53a2948f8f5eb10d764ecc24c198'
...
#         rows = rows[3:5] # limit rows when testing to save time
    ...
    def parseEnglishAuthorPage(self, response):
        ...
        request = Request(self.germanBaseUrl + item['href'], callback=self.parseGermanPage)
        request.meta['data'] = item

        return request

    def parseGermanPage(self, response):
        item = response.meta['data']

        item['germanRating'] = response.xpath("//span[@class='rate-points']/span/text()").extract()

        # html
        f = open(self.germanDocumentsFolder + item["scpId"] + ".html", "w")
        f.write(response.text)
        f.close()

        # txt
        f = open(self.germanDocumentsFolder + item["scpId"] + ".txt", "w")
        for paragraph in response.xpath("//div[@id='page-content']//text()"):
            f.write(paragraph.extract())
        f.close
        ...
        # open revision history tab and get the initial author + date
        # important that you have wikidot_token7 in both the formdata and the cookie!
        request = FormRequest(self.germanBaseUrl + '/ajax-module-connector.php',
                                callback=self.parseGermanAuthorAndDate,
                                method='POST',
                                formdata={'page_id': pageId, 'moduleName': 'history/PageRevisionListModule', 'wikidot_token7': self.germanToken, 'page': '1', 'perpage': '20'},
                                cookies={"wikidot_token7": self.germanToken})
        request.meta['data'] = item

        return request

    def parseGermanAuthorAndDate(self, response):
        item = response.meta['data']
        ...
        item['germanDate'] = initialRevision.xpath('td[6]/span/text()').extract_first()
        item['germanAuthor'] = initialRevision.xpath('td[5]/span/a[2]/text()').extract_first()

        # now move to the german author page to get their karma
        authorUrl = initialRevision.xpath('td[5]/span/a[2]/@href').extract_first()
        request = Request(authorUrl, callback=self.parseGermanAuthorPage)
        request.meta['data'] = item

        return request

    def parseGermanAuthorPage(self, response):
        item = response.meta['data']
        ...
        item['germanAuthorKarma'] = karmaString
