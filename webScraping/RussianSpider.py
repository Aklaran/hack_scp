...
class RussianSpider(Spider):
    name = 'russianSpider'
    allowed_domains = ['scp-int.wikidot.com', 'www.wikidot.com', 'scpfoundation.net'] # don't forget to update this
    start_urls = [ "http://scp-int.wikidot.com/ru-hub" ]

    englishDocumentsFolder = "englishFromRussian/"
    russianDocumentsFolder = "russianOriginals/"
    englishBaseUrl = "http://scp-int.wikidot.com"
    russianBaseUrl = "http://scpfoundation.net"
    englishToken = '610da9ba2f8fa1ca5ef14c7488cdfc16'
    russianToken = 'd6be53a2948f8f5eb10d764ecc24c198'
...
#         rows = rows[3:5] # limit rows when testing to save time
    ...
    def parseEnglishAuthorPage(self, response):
        ...
        request = Request(self.russianBaseUrl + item['href'], callback=self.parseRussianPage)
        request.meta['data'] = item

        return request

    def parseRussianPage(self, response):
        item = response.meta['data']

        item['russianRating'] = response.xpath("//span[@class='rate-points']/span/text()").extract()

        # html
        f = open(self.russianDocumentsFolder + item["scpId"] + ".html", "w")
        f.write(response.text)
        f.close()

        # txt
        f = open(self.russianDocumentsFolder + item["scpId"] + ".txt", "w")
        for paragraph in response.xpath("//div[@id='page-content']//text()"):
            f.write(paragraph.extract())
        f.close
        ...
        # open revision history tab and get the initial author + date
        # important that you have wikidot_token7 in both the formdata and the cookie!
        request = FormRequest(self.russianBaseUrl + '/ajax-module-connector.php',
                                callback=self.parseRussianAuthorAndDate,
                                method='POST',
                                formdata={'page_id': pageId, 'moduleName': 'history/PageRevisionListModule', 'wikidot_token7': self.russianToken, 'page': '1', 'perpage': '20'},
                                cookies={"wikidot_token7": self.russianToken})
        request.meta['data'] = item

        return request

    def parseRussianAuthorAndDate(self, response):
        item = response.meta['data']
        ...
        item['russianDate'] = initialRevision.xpath('td[6]/span/text()').extract_first()
        item['russianAuthor'] = initialRevision.xpath('td[5]/span/a[2]/text()').extract_first()

        # now move to the russian author page to get their karma
        authorUrl = initialRevision.xpath('td[5]/span/a[2]/@href').extract_first()
        request = Request(authorUrl, callback=self.parseRussianAuthorPage)
        request.meta['data'] = item

        return request

    def parseRussianAuthorPage(self, response):
        item = response.meta['data']
        ...
        item['russianAuthorKarma'] = karmaString
