...
class ChineseSpider(Spider):
    name = 'chineseSpider'
    allowed_domains = ['scp-int.wikidot.com', 'ko.scp-wiki.net', 'www.wikidot.com', 'scp-wiki-cn.wikidot.com'] # don't forget to update this
    start_urls = [ "http://scp-int.wikidot.com/cn-hub" ]

    englishDocumentsFolder = "englishFromChinese/"
    chineseDocumentsFolder = "chineseOriginals/"
    englishBaseUrl = "http://scp-int.wikidot.com"
    chineseBaseUrl = "http://scp-wiki-cn.wikidot.com"
    englishToken = '610da9ba2f8fa1ca5ef14c7488cdfc16'
    chineseToken = 'd6be53a2948f8f5eb10d764ecc24c198'
...
#         rows = rows[3:5] # limit rows when testing to save time
    ...
    def parseEnglishAuthorPage(self, response):
        ...
        request = Request(self.chineseBaseUrl + item['href'], callback=self.parseChinesePage)
        request.meta['data'] = item

        return request

    def parseChinesePage(self, response):
        item = response.meta['data']

        item['chineseRating'] = response.xpath("//span[@class='rate-points']/span/text()").extract()

        # html
        f = open(self.chineseDocumentsFolder + item["scpId"] + ".html", "w")
        f.write(response.text)
        f.close()

        # txt
        f = open(self.chineseDocumentsFolder + item["scpId"] + ".txt", "w")
        for paragraph in response.xpath("//div[@id='page-content']//text()"):
            f.write(paragraph.extract())
        f.close
        ...
        # open revision history tab and get the initial author + date
        # important that you have wikidot_token7 in both the formdata and the cookie!
        request = FormRequest(self.chineseBaseUrl + '/ajax-module-connector.php',
                                callback=self.parseChineseAuthorAndDate,
                                method='POST',
                                formdata={'page_id': pageId, 'moduleName': 'history/PageRevisionListModule', 'wikidot_token7': self.chineseToken, 'page': '1', 'perpage': '20'},
                                cookies={"wikidot_token7": self.chineseToken})
        request.meta['data'] = item

        return request

    def parseChineseAuthorAndDate(self, response):
        item = response.meta['data']
        ...
        item['chineseDate'] = initialRevision.xpath('td[6]/span/text()').extract_first()
        item['chineseAuthor'] = initialRevision.xpath('td[5]/span/a[2]/text()').extract_first()

        # now move to the chinese author page to get their karma
        authorUrl = initialRevision.xpath('td[5]/span/a[2]/@href').extract_first()
        request = Request(authorUrl, callback=self.parseChineseAuthorPage)
        request.meta['data'] = item

        return request

    def parseChineseAuthorPage(self, response):
        item = response.meta['data']
        ...
        item['chineseAuthorKarma'] = karmaString
