...
class JapaneseSpider(Spider):
    name = 'japaneseSpider'
    allowed_domains = ['scp-int.wikidot.com', 'fondazionescp.wikidot.com', 'www.wikidot.com', 'ja.scp-wiki.net'] # don't forget to update this
    start_urls = [ "http://scp-int.wikidot.com/jp-hub" ]

    englishDocumentsFolder = "englishFromJapanese/"
    japaneseDocumentsFolder = "japaneseOriginals/"
    englishBaseUrl = "http://scp-int.wikidot.com"
    japaneseBaseUrl = "http://ja.scp-wiki.net"
    englishToken = '610da9ba2f8fa1ca5ef14c7488cdfc16'
    japaneseToken = '6bf45da9792159b591f8b6be75828abe'
    ...
    def parse(self, response):
        ...
        logging.log(logging.DEBUG, "got first response")
#         rows = rows[3:5] # limit rows when testing to save time
        items = []
        for row in rows:
            logging.log(logging.DEBUG, "going through a row")
            # the data to save
            item = {}
    ...
    def parseEnglishAuthorPage(self, response):
        ...
        # now get the japanese
        # I don't know why this has to be done from this function, but it called a "hackathon" for a reason, right?
        request = Request(self.japaneseBaseUrl + item['href'], callback=self.parseJapanesePage)
        request.meta['data'] = item

        return request

    def parseJapanesePage(self, response):
        item = response.meta['data']

        item['japaneseRating'] = response.xpath("//span[@class='rate-points']/span/text()").extract()

        # html
        f = open(self.japaneseDocumentsFolder + item["scpId"] + ".html", "w")
        f.write(response.text)
        f.close()

        # txt
        f = open(self.japaneseDocumentsFolder + item["scpId"] + ".txt", "w")
        for paragraph in response.xpath("//div[@id='page-content']//text()"):
            f.write(paragraph.extract())
        f.close
        ...
        # open revision history tab and get the initial author + date
        # important that you have wikidot_token7 in both the formdata and the cookie!
        request = FormRequest(self.japaneseBaseUrl + '/ajax-module-connector.php',
                                callback=self.parseJapaneseAuthorAndDate,
                                method='POST',
                                formdata={'page_id': pageId, 'moduleName': 'history/PageRevisionListModule', 'wikidot_token7': self.japaneseToken, 'page': '1', 'perpage': '20'},
                                cookies={"wikidot_token7": self.japaneseToken})
        request.meta['data'] = item

        return request

    def parseJapaneseAuthorAndDate(self, response):
        item = response.meta['data']
        ...
        item['japaneseDate'] = initialRevision.xpath('td[6]/span/text()').extract_first()
        item['japaneseAuthor'] = initialRevision.xpath('td[5]/span/a[2]/text()').extract_first()

        # now move to the japanese author page to get their karma
        authorUrl = initialRevision.xpath('td[5]/span/a[2]/@href').extract_first()
        request = Request(authorUrl, callback=self.parseJapaneseAuthorPage)
        request.meta['data'] = item

        return request

    def parseJapaneseAuthorPage(self, response):
        item = response.meta['data']
        ...
        if karmaSearch:
            ...
            karmaString = 'Not found'
        item['japaneseAuthorKarma'] = karmaString
