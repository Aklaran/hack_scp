...
class FrenchSpider(Spider):
    name = 'frenchSpider'
    allowed_domains = ['scp-int.wikidot.com', 'www.wikidot.com', 'fondationscp.wikidot.com'] # don't forget to update this
    start_urls = [ "http://scp-int.wikidot.com/fr-hub" ]

    englishDocumentsFolder = "englishFromFrench/"
    frenchDocumentsFolder = "frenchOriginals/"
    englishBaseUrl = "http://scp-int.wikidot.com"
    frenchBaseUrl = "http://fondationscp.wikidot.com"
    englishToken = '610da9ba2f8fa1ca5ef14c7488cdfc16'
    frenchToken = 'd6be53a2948f8f5eb10d764ecc24c198'
    ...
    def parse(self, response):
        rows = response.xpath("/html/body/div[1]/div/div[1]/div/div[2]/div[2]/div[4]/div[2]/p[1]/a")
#         rows = rows[3:5] # limit rows when testing to save time
#         logging.log(logging.DEBUG, rows)
#         logging.log(logging.DEBUG, "got " + str(len(rows)) + " responses")
        items = []
        for row in rows:
            # the data to save
            item = {}
...
#             if row.xpath("./a").extract(): # check for annoying things
            item['scpId'] = row.xpath("./text()").extract()[0] # indexing at 0 appears to get the string
            item['href'] = row.xpath("./@href").extract()[0]
    ...
    def parseEnglishAuthorPage(self, response):
        ...
        request = Request(self.frenchBaseUrl + item['href'], callback=self.parseFrenchPage)
        request.meta['data'] = item

        return request

    def parseFrenchPage(self, response):
        item = response.meta['data']

        item['frenchRating'] = response.xpath("//span[@class='rate-points']/span/text()").extract()

        # html
        f = open(self.frenchDocumentsFolder + item["scpId"] + ".html", "w")
        f.write(response.text)
        f.close()

        # txt
        f = open(self.frenchDocumentsFolder + item["scpId"] + ".txt", "w")
        for paragraph in response.xpath("//div[@id='page-content']//text()"):
            f.write(paragraph.extract())
        f.close
        ...
        # open revision history tab and get the initial author + date
        # important that you have wikidot_token7 in both the formdata and the cookie!
        request = FormRequest(self.frenchBaseUrl + '/ajax-module-connector.php',
                                callback=self.parseFrenchAuthorAndDate,
                                method='POST',
                                formdata={'page_id': pageId, 'moduleName': 'history/PageRevisionListModule', 'wikidot_token7': self.frenchToken, 'page': '1', 'perpage': '20'},
                                cookies={"wikidot_token7": self.frenchToken})
        request.meta['data'] = item

        return request

    def parseFrenchAuthorAndDate(self, response):
        item = response.meta['data']
        ...
        item['frenchDate'] = initialRevision.xpath('td[6]/span/text()').extract_first()
        item['frenchAuthor'] = initialRevision.xpath('td[5]/span/a[2]/text()').extract_first()

        # now move to the french author page to get their karma
        authorUrl = initialRevision.xpath('td[5]/span/a[2]/@href').extract_first()
        request = Request(authorUrl, callback=self.parseFrenchAuthorPage)
        request.meta['data'] = item

        return request

    def parseFrenchAuthorPage(self, response):
        item = response.meta['data']
        ...
        item['frenchAuthorKarma'] = karmaString
