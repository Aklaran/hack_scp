...
class PortugueseSpider(Spider):
    name = 'portugueseSpider'
    allowed_domains = ['scp-int.wikidot.com', 'www.wikidot.com', 'scp-pt-br.wikidot.com'] # don't forget to update this
    start_urls = [ "http://scp-int.wikidot.com/pt-hub" ]

    englishDocumentsFolder = "englishFromPortuguese/"
    portugueseDocumentsFolder = "portugueseOriginals/"
    englishBaseUrl = "http://scp-int.wikidot.com"
    portugueseBaseUrl = "http://scp-pt-br.wikidot.com"
    englishToken = '610da9ba2f8fa1ca5ef14c7488cdfc16'
    portugueseToken = 'd6be53a2948f8f5eb10d764ecc24c198'
    ...
    def parse(self, response):
        rows = response.xpath("//div[@id='page-content']/div[@class='content-panel standalone series'][1]/ul[2]/li")
#         rows = rows[3:5] # limit rows when testing to save time
    ...
    def parseEnglishAuthorPage(self, response):
        ...
        # now get the Portuguese
        # I don't know why this has to be done from this function, but it called a "hackathon" for a reason, right?
        request = Request(self.portugueseBaseUrl + item['href'], callback=self.parsePortuguesePage)
        request.meta['data'] = item

        return request

    def parsePortuguesePage(self, response):
        item = response.meta['data']

        item['portugueseRating'] = response.xpath("//span[@class='rate-points']/span/text()").extract()

        # html
        f = open(self.portugueseDocumentsFolder + item["scpId"] + ".html", "w")
        f.write(response.text)
        f.close()

        # txt
        f = open(self.portugueseDocumentsFolder + item["scpId"] + ".txt", "w")
        for paragraph in response.xpath("//div[@id='page-content']//text()"):
            f.write(paragraph.extract())
        f.close
        ...
        # open revision history tab and get the initial author + date
        # important that you have wikidot_token7 in both the formdata and the cookie!
        request = FormRequest(self.portugueseBaseUrl + '/ajax-module-connector.php',
                                callback=self.parsePortugueseAuthorAndDate,
                                method='POST',
                                formdata={'page_id': pageId, 'moduleName': 'history/PageRevisionListModule', 'wikidot_token7': self.portugueseToken, 'page': '1', 'perpage': '20'},
                                cookies={"wikidot_token7": self.portugueseToken})
        request.meta['data'] = item

        return request

    def parsePortugueseAuthorAndDate(self, response):
        item = response.meta['data']
        ...
        item['portugueseDate'] = initialRevision.xpath('td[6]/span/text()').extract_first()
        item['portugueseAuthor'] = initialRevision.xpath('td[5]/span/a[2]/text()').extract_first()

        # now move to the portuguese author page to get their karma
        authorUrl = initialRevision.xpath('td[5]/span/a[2]/@href').extract_first()
        request = Request(authorUrl, callback=self.parsePortugueseAuthorPage)
        request.meta['data'] = item

        return request

    def parsePortugueseAuthorPage(self, response):
        item = response.meta['data']
        ...
        item['portugueseAuthorKarma'] = karmaString
